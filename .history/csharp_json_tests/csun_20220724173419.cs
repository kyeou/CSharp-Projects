    ...
    public class csun
    {
        public static async Task Main()
        {
           ...
           // Classes? classes = await JsonSerializer.DeserializeAsync<Classes>(await stringTask);
            ...
            List<Class1> complete_listing = new List<Class1>(fall_listing?.classes.Count + spring_listing?.classes.Count );
            complete_listing.AddRange(fall_listing?.classes?);
            complete_listing.AddRange(spring_listing?.classes?);




            // foreach(var course in rtob?.classes) {
            //      Console.WriteLine(course.catalog_number);
            // }
