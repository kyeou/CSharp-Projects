...
namespace csun
{
    ...
    public class csun
    {
        ...
        public static async Task ShowSchedule()
        {
            ...
            fall_listing.classes.OrderBy(x => x.catalog_number).ToList().ForEach(x => Console.WriteLine($"{x.class_number} - {x.catalog_number} - {x.title} - {x.units}"));

            int current_class = 0;
            foreach (Class c in fall_listing.classes) {
                if (!(current_class.Equals(catalog_number))) {
        ...
        section_string.add("\t\t" + str(course["enrollment_cap"] - course["enrollment_count"]))
        section_string.add("\t\t    " +
                              (c.meetings[0]["start_time"])[0:2] + ":" +
                              (c.meetings[0]["start_time"])[2:4] 
                              + " - " +
                              (c.meetings[0]["end_time"])[0:2] + ":" +
                              (c.meetings[0]["end_time"])[2:4])
                }//end ML if
