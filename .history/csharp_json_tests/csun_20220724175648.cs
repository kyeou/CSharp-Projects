...
//              }
 ...
 List<Class1> complete_listing2 = complete_listing.Distinct().ToList();
complete_listing2.ToList().OrderBy(x=>x.catalog_number).ToList().ForEach(x=> Console.WriteLine($"{x.catalog_number} - {x.title} - {x.units} - {x.class_type}"));
