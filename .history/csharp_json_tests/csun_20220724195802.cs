...
namespace csun
{
    ...
    public class csun
    {
        ...
        public static async Task ShowSchedule()
        {
            ...
            fall_listing.classes.DistinctBy(x => x.catalog_number).OrderBy(x => x.catalog_number).ToList().ForEach(x => Console.WriteLine($"{x.class_number}{x.catalog_number} - {x.title} - {x.units}"));
        }
